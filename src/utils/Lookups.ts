...
export function organisationNameByUuid(
  uuid: string,
  organisations?: Organization[]
) {
  const organisation =
    organisations && organisations.find((org) => org.uuid === uuid)
...
export function enrichSubscription(
  ...
  subscription.customer_name = organisationNameByUuid(
    subscription.customer_id!,
    organisations
  )
  subscription.product = productById(subscription.product_id!, products)
  subscription.end_date_epoch = subscription.end_date
    ? new Date(subscription.end_date).getTime()
    : 0
  subscription.start_date_epoch = subscription.start_date
...
export function renderDateTime(epoch: number) {
  return isEmpty(epoch)
    ? ''
    : new Date(epoch * 1000).toLocaleString('nl-NL') + ' CET'
}

export function renderDate(epoch: number) {
  return isEmpty(epoch)
    ? ''
    : new Date(epoch * 1000).toLocaleDateString('nl-NL') + ' CET'
}
